...
function Brand({ onClick }: { onClick?: () => void }) {
  return (
    <a className="brand" href="#top" aria-label="PDT Labs home" onClick={onClick}>
      <span className="brand-mark" aria-hidden="true"><b>PDT</b><i>/</i></span>
      <span className="brand-name">Labs</span>
    </a>
  );
}
  ...
  return (
    ...
    >
      <a className="project-image" href={project.liveUrl} target="_blank" rel="noreferrer">
        ...
        />
        <span>Visit site <ArrowUpRight size={18} weight="bold" /></span>
      </a>
      <div className="project-copy">
        ...
        <div className="project-links">
          <a className="project-live-link" href={project.liveUrl} target="_blank" rel="noreferrer">Visit live site <ArrowUpRight size={15} /></a>
          <a className="project-source-link" href={project.repoUrl} target="_blank" rel="noreferrer"><GithubLogo size={17} /> View source</a>
        </div>
      </div>
    </motion.article>
  );
      ...
      <main>
        ...
        <section id="process" className="process section-rule" aria-labelledby="process-title">
          <div className="process-heading section-pad">
            ...
            <h2 id="process-title">How your project gets shipped.</h2>
          </div>
        ...
        <section id="pricing" className="pricing section-pad section-rule" aria-labelledby="pricing-title">
          <div className="pricing-top">
            <div>
              ...
              <h2 id="pricing-title">Packages and pricing.</h2>
            </div>
          ...
          <div className="pricing-grid">
            {services.filter((service) => service.name !== "Product rescue").map((service, index) => (
              <article className={`pricing-card ${index === 2 ? "pricing-card-featured" : ""}`} key={service.name}>
                ...
                <p className="pricing-purpose">{["Validate an idea", "Present your business", "Launch the core product"][index]}</p>
                <div className="price-line">
                  <small>Founding price</small>
                  <div className="price-values"><del>{service.standardPrice}</del><strong>{service.price}</strong></div>
                  <span>USD</span>
                </div>
                <p className="pricing-summary">{service.summary}</p>
                <ul>
                  {service.deliverables.map((item) => <li key={item}><Check size={16} weight="bold" /> {item}</li>)}
                </ul>
        ...
        <section id="faq" className="faq section-pad section-rule" aria-labelledby="faq-title">
          <div className="faq-heading">
            ...
            <h2 id="faq-title">Questions before we start.</h2>
          </div>
